...
FEditorUIManager::FEditorUIManager(const ImGuiIO& InIO)
	: mGuiInputField()
{
	mPanelWidth = InIO.DisplaySize.x * MIN_WIDTH_RATIO;
	mViewportX = 0;
	mViewportY = 0;
...
void FEditorUIManager::UpdateGUI(const FGuiReference& guiReference)
{
	//ImGui
...
void FEditorUIManager::updateControlPanelGUI(const FGuiReference& guiReference)
{
	...
	if (ImGui::Button("Spawn"))
	{
		for (int32 i = 0; i < mGuiInputField.SpawnCount; ++i)
		{
			...
			if (NewActor)
			{
				...
				guiReference.SceneManager->GetCurrentWorld()->AddActor(NewActor);
			}
		}
	}
	ImGui::SameLine();
	...
	if (ImGui::Button("Save scene"))
	{
		try
		{
			...
			{
				guiReference.SceneManager->SaveScene(selectedPath.value(), *guiReference.FileManager);
	...
	if (ImGui::Button("Load scene"))
	{
		try
		{
			...
			if (selectedPath.has_value())
			{
				guiReference.SceneManager->LoadScene(
					selectedPath.value(),
				...
				for (AActor* Actor : guiReference.SceneManager->GetCurrentWorld()->GetActors())
				{
...
void FEditorUIManager::updatePropertyWindowGUI(const FGuiReference& guiReference)
{
	ImGuiIO& io = ImGui::GetIO();
	...
	if (AActor* SelectedActor = guiReference.SceneManager->GetSelectedActor())
	{
		// Temporary variables to hold the values for ImGui input fields
		const FTransform& originalTransform = SelectedActor->GetTransform();
		...
		{
			SelectedActor->SetLocation(translationInput);
		}
		if (ImGui::DragFloat3("Rotation", &rotationInput.x, 0.1f))
		{
			SelectedActor->SetRotation({
		...
		{
			SelectedActor->SetScale(scaleInput);
		}
...
void FEditorUIManager::updateObjectListPanelGUI(const FGuiReference& guiReference)
{
	ImGuiIO& io = ImGui::GetIO();
	...
	ImGui::Begin("Object List Panel", nullptr, flags);
	{
			...
			ImGuiChildFlags_Borders))
		{
			...
			if (bDeleteActorOrNull != nullptr)
			{
				...
				if (guiReference.SceneManager->GetSelectedActor() != nullptr && guiReference.SceneManager->GetSelectedActor()->UUID == deleteActor->UUID)
				{
					guiReference.SceneManager->ResetSelectedActor();
				}

				assert(guiReference.SceneManager->GetCurrentWorld() != nullptr);
				guiReference.SceneManager->GetCurrentWorld()->RemoveActor(deleteActor->UUID);
